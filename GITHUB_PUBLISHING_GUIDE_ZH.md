...
```bash
./publish_release.sh 0.8.1
```

确认输出后自动提交指定源码和验证文件、推送当前分支、建立 `v0.8.1` Release 并上传 paclet：

```bash
./publish_release.sh 0.8.1 --publish
```

也可以指定发布说明：

```bash
./publish_release.sh 0.8.1 --publish --notes-file RELEASE_NOTES_0.8.1.md
```
